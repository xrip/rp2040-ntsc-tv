...
The low-level NTSC signal generator is the portable `drivers/ntsc-tv` module. The root `ntsc-tv.c` file provides the video content to be displayed.
...
*   The project is structured with a clear separation between the signal generation module (`drivers/ntsc-tv`) and the application logic (`ntsc-tv.c`).
